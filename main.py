...
def rep_with_upper(string_value):
    values = list(string_value)
    words = random.choice(values)
    new_word = ""

    for i in range(len(values)):
        if values[i] == words:
            values[i] = words.upper()

    new_word = new_word.join(values)
    return new_word
print(rep_with_upper(passGen(user_number)))
...
print(swapletters(rep_with_upper(passGen(user_number))))

#q3
def serach_letter(letter,word):
    if letter in word:
        return True
    return False

letter=input("Enter the searching letter:")
print(serach_letter( letter,passGen(user_number)))
